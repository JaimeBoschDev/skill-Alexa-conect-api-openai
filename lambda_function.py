...
# Clave de OpenAI
openai.api_key = "Reemplaza por tu API Key"  # Reemplaza con tu clave real
...
def obtener_respuesta_openai(messages: list) -> str:
    ...
    try:
        ...
        if not texto:
            texto = "Lo siento RAINER, no tengo una respuesta para eso."
        return texto
    except Exception as e:
        logger.error(f"Error al comunicarse con OpenAI: {e}", exc_info=True)
        return "Lo siento, no puedo obtener una respuesta en este momento de MARLEY."
...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech_text = "A tus ordenes Fundador."
        return (
